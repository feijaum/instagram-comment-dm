import { z } from "zod";
import { createSession, getSessionUser, isLoginRateLimited, isSameOrigin, recordLoginAttempt, revokeSession, verifyPassword } from "./auth";

export interface Env { DB: D1Database; INSTAGRAM_TOKEN_ENCRYPTION_KEY?: string; SESSION_SECRET?: string; }
...
function genericLoginFailure(): Response { return json({ error: "E-mail ou senha inválidos." }, { status: 401 }); }
...
async function login(request: Request, env: Env): Promise<Response> {
  ...
  const user = await env.DB.prepare("SELECT id, email, password_hash, is_active FROM users WHERE email = ?1 LIMIT 1")
    .bind(email.toLowerCase()).first<{ id: string; email: string; password_hash: string; is_active: number }>();
  if (!user || !user.is_active) { await recordLoginAttempt(env.DB, email, false); return genericLoginFailure(); }
...
async function logout(request: Request, env: Env): Promise<Response> {
  ...
  if (user) await env.DB.prepare("INSERT INTO audit_logs (id, user_id, action, resource_type, outcome) VALUES (?1, ?2, 'logout', 'session', 'success')")
    .bind(crypto.randomUUID(), user.id).run();
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (request.method === "GET" && url.pathname === "/api/health") return json({ status: "ok", service: "instagram-comment-dm" });
    ...
    return env.DB ? new Response("Aplicação pronta para o shell do Worker.", { status: 200 }) : new Response("Configuração do banco ausente.", { status: 503 });
  },
};
